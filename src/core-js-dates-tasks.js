...
function getNextFriday(date) {
  ...
  return new Date(
    temp.setDate(temp.getDate() + ((7 - temp.getDay() + 5) % 7 || 7))
  );
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const dif = new Date(dateEnd).getTime() - new Date(dateStart).getTime();
  return Math.ceil(dif / day) + 1;
}
...
function getWeekNumberByDate(date) {
  ...
  const dif = (date - first) / 86400000;
  return Math.ceil((dif + (first.getDay() || 7)) / 7);
}
...
function getNextFridayThe13th(date) {
  ...
  for (let i = new Date(date).getMonth(); i < 12; i += 1) {
    const temp = new Date(day.getFullYear(), i, 13);
    if (temp.getDay() === 5) return temp;
  }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  for (let i = s; i <= e; i += day) {
    const temp = new Date(i);
    arr.push(temp.toLocaleDateString('es-CL'));
  }

  let i = 0;

  for (; i < arr.length; i += countOffDays) {
    let j = 0;
    for (; j < countWorkDays; j += 1) if (arr[i + j]) res.push(arr[i + j]);
    i += j;
  }
